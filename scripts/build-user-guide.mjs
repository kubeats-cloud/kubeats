...
children.push(
  ...
  h3("Meeting purposes"),
  p("The list reps choose from when planning a visit. Add one with the box at the bottom — say what it counts as, and for a session or a campus visit whether it SETS one for later or COMPLETES one held today. Changing this list changes what every rep sees."),
  p("There is no delete. Retire a purpose you no longer want and it stops being offered, while every visit already planned under it stays readable; Restore brings it back. Deleting one would quietly stop a weekly number being earned, with nothing on screen to say so."),
